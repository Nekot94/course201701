...
friends = []
your_id = '381462099'
data = urlopen("https://api.vk.com/method/friends.get?user_id="+your_id+'&fields=nickname').read().decode("utf-8")
data = loads(data)
print(data)

friends_response = data["response"]
friends_id = [el["user_id"] for el in friends_response]
friends_name = [name['first_name'] + " " + name['last_name'] for name in friends_response]
print(friends_id)
print(friends_name)
