# file1 = open("arslan_nevirus.txt", "wt", encoding="utf-8")
# file1.write("Мощь зеленого великана безгранична\n")
# file1.write("Саша Молников\n")
# print("все что-нИужно", file=file1)
# file1.close()

# file1 = open("arslan_nevirus.txt", "rt", encoding="utf-8")
# # text = file1.read(8)
# line1 = file1.readline()
# line2 = file1.readline()
# file1.close()
# print(line1 + line2)
# # print(text)

# file1 = open("arslan_nevirus.txt", "rt", encoding="utf-8")
# for line in file1:
# 	print(line, end="", sep="+")
# file1.close()

# text = """Sweet dreams all made of this
# Who am I to disagree"""

# with open("sweet.txt", "wt", encoding="utf-8") as f:
# 	f.write(text)

# text2 = """\nI traveled the world and the seven seas
# Everbody is looking for something""" 


# with open("sweet.txt", "at", encoding="utf-8") as f:
#  	f.write(text2)






